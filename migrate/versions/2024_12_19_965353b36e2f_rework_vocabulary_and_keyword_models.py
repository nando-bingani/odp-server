...
Revision ID: 965353b36e2f
Revises: 75fd0f193787
Create Date: 2024-12-19 08:50:01.266776
...
def upgrade():
    ...
    op.execute("create collation if not exists naturalsort (provider = icu, locale = 'en@colNumeric=yes')")
    ...
    op.create_table('keyword',
                    ...
                    sa.ForeignKeyConstraint(['vocabulary_id', 'parent_id'], ['keyword.vocabulary_id', 'keyword.id'], name='keyword_parent_fkey',
                                            ondelete='RESTRICT'),
                    sa.ForeignKeyConstraint(['vocabulary_id'], ['vocabulary.id'], ondelete='CASCADE'),
                    sa.PrimaryKeyConstraint('vocabulary_id', 'id'),
    ...
    op.drop_column('vocabulary', 'scope_id')
    op.drop_column('vocabulary', 'scope_type')
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - adjusted ###
    op.add_column('vocabulary', sa.Column('scope_type', postgresql.ENUM(name='scopetype', create_type=False),
                                          server_default=sa.text("'odp'::scopetype"), autoincrement=False, nullable=False))
    op.add_column('vocabulary', sa.Column('scope_id', sa.VARCHAR(),
                                          server_default=sa.text("'odp.vocabulary:read'::character varying"), autoincrement=False, nullable=False))
    ...
    # skip downgrading the check constraint
    # op.drop_constraint('vocabulary_schema_type_check', 'vocabulary', type_='check')
